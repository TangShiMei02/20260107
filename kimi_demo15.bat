...
for /f "tokens=*" %%r in ('git remote') do (
    set /a count+=1
    set "remote_!count!=%%r"
    set "remote_list=!remote_list!%%r "
    echo   !count! = %%r
)
endlocal & set "count=%count%" & set "remote_list=%remote_list%"

:: 让用户选择远程
set /p "remote_choice=请选择要推送的远程 [1-%count%]: "
if "%remote_choice%"=="" set "remote_choice=1"
:: 根据选择获取远程名称
for /f "tokens=*" %%c in ('echo !remote_%remote_choice%!') do set "REMOTE_NAME=%%c"

:: ========== 选择推送协议（保留原逻辑，但增加提示）==========
echo.
echo 推送协议选择（仅用于验证/转换远程URL协议）：
echo   按下 H = HTTPS
echo   按下 S = SSH
choice /C HS /N /M "请选择: "
if %errorlevel%==1 (
...
:: ========== 优化协议判断逻辑 ==========
:: 获取选中远程的URL（修复空格问题）
for /f "tokens=2" %%u in ('git remote -v ^| findstr /i /c:"%REMOTE_NAME%.*fetch"') do set "CURRENT_URL=%%u"
if "%CURRENT_URL%"=="" (
    echo [ERROR] 无法获取远程地址，请检查配置
    pause
    goto MAIN_MENU
)
...
if not "%CURRENT_PROTO%"=="%PUSH_PROTO%" (
    echo [WARN] 当前远程[%REMOTE_NAME%]的URL协议是%CURRENT_PROTO%，你选择的推送协议是%PUSH_PROTO%
    echo [INFO] 正在自动转换远程[%REMOTE_NAME%]的URL协议...
    setlocal enabledelayedexpansion
    if "%PUSH_PROTO%"=="https" (
        set "NEW_URL=!CURRENT_URL:git@github.com:=https://github.com/!"
        set "NEW_URL=!NEW_URL:.git=!.git"
    ) else (
        set "NEW_URL=!CURRENT_URL:https://github.com/=git@github.com:!"
        set "NEW_URL=!NEW_URL:.git=!.git"
    )
    git remote set-url %REMOTE_NAME% "!NEW_URL!"
    echo [INFO] 已转换为: !NEW_URL!
    endlocal
) else (
    echo [INFO] 协议匹配，无需转换
)

echo [INFO] 确认推送至：远程[%REMOTE_NAME%] 分支[%BRANCH%]
echo.

:: ========== 保留原有的排除node_modules、提交、推送逻辑 ==========
:: 排除 node_modules
