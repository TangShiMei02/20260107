@echo off
:: GitHub 超级工具箱 - 优化版（必须ANSI编码）
chcp 936 >nul
title GitHub 超级工具箱 Pro
color 0F
...
echo.
echo "协议选择："
echo "1. HTTPS (推荐新手，无需SSH密钥)"
echo "2. SSH (需要提前配置SSH密钥)"
choice /C 12 /N /M "请选择: "
if %errorlevel%==1 (
...
if %errorlevel%==1 set "REMOTE_NAME=https" else set "REMOTE_NAME=ssh"

git remote | findstr "^%REMOTE_NAME%$" >nul
if %errorlevel% neq 0 (
    echo "[WARN] 未找到 %REMOTE_NAME% 远程"
    git remote -v | findstr "origin.*fetch" >nul
    if %errorlevel% neq 0 (
        echo "[ERROR] 未配置任何远程仓库"
        pause
        goto MAIN_MENU
    )
    echo "尝试从 origin 自动转换..."
    for /f "tokens=2" %%u in ('git remote -v ^| findstr /i origin.*fetch') do set "ORIGIN_URL=%%u"
    setlocal enabledelayedexpansion
    if "%REMOTE_NAME%"=="https" (
        set "NEW_URL=!ORIGIN_URL:git@github.com:=https://github.com/!"
    ) else (
        set "NEW_URL=!ORIGIN_URL:https://github.com/=git@github.com:!"
    )
    git remote add %REMOTE_NAME% !NEW_URL!
    echo "[INFO] 已自动添加 %REMOTE_NAME% 远程"
    endlocal
)
